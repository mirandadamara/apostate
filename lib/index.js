...
export default function( initializer = {} ) {
	...
	signals.observeOn( Rx.Scheduler.default ).subscribe( function( signal ) {
		...
		chain( signal.params, actions.map( wrap( signal ) ) )		// Without observing, actions would not be executed (since there is no observer).
			.subscribe(
				function( val ) {},
				function( err ) {
					console.error( err );
		...
		return function( action ) {
			return function( input ) {
				...
				return Rx.Observable.create( function( observer ) {
					updates
						.take( 1 )
						.map( state => mutate( state, action, input, signal ) )
						.concatMap( value => value )
						.tapOnCompleted( function() {
							console.log( "ACTION:", signal.name, now() - start );
						})
						.subscribe( observer );
				});
			}
