...
export default function( initializer = {} ) {
	...
	// React to signals.
	signals.observeOn( Rx.Scheduler.default ).subscribe( function( signal ) {
		signal.latency = now() - signal.time;

		console.log( "DISPATCH:", signal.name, signal );

		perf.start( 'DISPATCH' );
		perf.push( 'LOOKUP' );
		let actions = handlers[signal.name] || Rx.Observable.empty();
		perf.pop();

		perf.push( 'WRAP' );
		actions = actions.map( wrap( signal ) );
		perf.pop();

		chain( signal.params, actions )		// Without observing, actions would not be executed (since there is no observer).
			.subscribe(
				function( val ) {},
				function( err ) {
					console.error( err );
				...
				function() {
					perf.end();
					console.log( "PERFORMANCE:", perf.report() );
				}
			);
		...
		return function ( action ) {
			return function( input ) {
				return Rx.Observable.create( function( observer ) {
					updates
						.take( 1 )
						.concatMap( state => mutate( state, action, input, signal ) )
						.subscribe( observer );
				});
			}
		}
	 ...
	 */
	function mutate( state, action, input, signal ) {
		let result;
		let update = state.withMutations( function( s ) {
			try {
				result = action( s, input, signal );
			}
			catch( err ) {
	...
	return {
		signal( name, actions = [] ) {
			...
			handlers[name] = Rx.Observable.fromArray( actions );
