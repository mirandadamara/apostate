...
export default function( initializer = {}, options = {} ) {
	...
	const signals = Rx.Observable.merge( frames().map( timestamp => ({ name: '$express$', params: { timestamp } }) ), queueCurrent.observeOn( Rx.Scheduler.currentThread ), queueBackground.observeOn( Rx.Scheduler.default ) );
